...
#+++++++++++++++++++++++++++++++++++++++++++
set simulation_name = 'MSI Guo Qi (2013)'
...
#+++++++++++++++++++++++++++++++++++++++++++
set my_annotation = 'Chabrier IMF; corrected to Planck cosmology by applying h 0.6777; approx. K-corr of mAB (Chilingarian+10)'
...
set tarsel_code_Galacticus = 'new_mags'
set tarsel_code_SAG = 'v3_new_mags'
set tarsel_code_SAGE = 'v3_1260'
...
set output_filename_code_default = 'mstar_gt_1e10'
...
# 122 119 116 113 110 107 104 101 98 95 92 89 86 83 80 77 75'
	...
	if ($SOFTLINK_CODE == A) then 
		...
		set MYSNAPS_1Gpc	= '107'
	else if ($SOFTLINK_CODE == I) then 
		set MYSNAPS_1Gpc	= '122'
	else if ($SOFTLINK_CODE == L) then 
		set MYSNAPS_1Gpc	= '45'
	else
...
#set filter_density = 'NFW_con,ssfr,sfr'
#Contrears CUTS
...
set filter_density_ngal = 41000
#3.4x10-4 Mpc-3 h3
set filter_density_cut_name = CMASS_density

set filter_density_select_highest = False
...
if ($output_filename_code_default =~ LOW*) then
...
foreach MODEL (`ls -d $PATH_TO_SOFTLINKS/*`)
	...
	foreach ISNAP ($MYSNAPS)
		...
		if ($NAME =~ *1Gpc* || $NAME =~ *400* && $NAME !~ SAG_* || $NAME =~ suss* || $NAME =~ LGAL*) then
		...
		foreach CATALOG ($key)
			...
			if ($SNAPNAME =~ *config* || $SNAPNAME =~ *idzred* || $SNAPNAME =~ *READ*) then
				...
				else if ($NAME =~ *400* || $NAME =~ *500*) then
					echo 'nr_rows= 50000000'   	>> $CONFIGFILE
				else
				...
				echo 'unit_corr_IMF= '$IMF_CORR				>> $CONFIGFILE
